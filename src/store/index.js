...
export const store = new Vuex.Store({
  state: {
    token: localStorage.getItem(process.env.LOCALSTORAGE_TOKEN_KEY) || null,
    user: {},
    tasks: [],
  },
  mutations: {
    authenticate(state, token, user) {
      ...
      localStorage.setItem(process.env.LOCALSTORAGE_TOKEN_KEY, token);
    },
    logout(state) {
      state.token = null;
      state.user = {};

      localStorage.setItem(process.env.LOCALSTORAGE_TOKEN_KEY, '');
    },
  },
  actions: {
